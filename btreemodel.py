...
class DivisionProblem:
    ...
    def __init__(self, varlist, dataset):
        ...
        # Get experience
        #self.experience = [set([])] * num # Just in case we want to increase efficiency
        #for instance in self.dataset:
        #    triple, oddIndex, weight = instance
        #    for i in range(3):
        #        self.experience[triple[i]].add((triple, oddIndex, weight))

    # Update rule
    def updateAssignment(self, triple, oddIndex, weight):
        raise Exception("Update rule need to be overridden.")

    # Observe one additional instance of training set
    #def observe(self, triple, oddIndex, weight):
    #    self.dataset.append((triple, oddIndex, weight))
    #    for i in range(3):
    #        self.experience[triple[i]].add((triple, oddIndex, weight))
...
class SmallUpdateDivision(DivisionProblem):
    ...
    def updateAssignment(self, triple, oddIndex, weight):
        ...
        if self.getReward() > oldReward and learn:
            return
        self.assignment[triple[indexchange]] = 1 - self.assignment[triple[indexchange]]

class LargeUpdateDivision(DivisionProblem):
