...
def getWordUniqueness(article1, article2, returntype=collections.Counter):
    result = returntype()
    for w in article1:
        result[w] = 1
    for w in article2:
        result[w] = 1
    for w in result:
        result[w] = math.log(float(article1[w] + 1) / float(article2[w] + 1))
    return result
...
def getWikiUniqueness(title1, title2, returntype=collections.Counter):
    return getWordUniqueness(getWordCountWiki(title1), getWordCountWiki(title2), returntype)

def getWikiCompareSpectrum(title1, title2, returntype=collections.Counter):
    maxi, mini = float('-inf'), float('inf')
    result = getWikiUniqueness(title1, title2, returntype)
    for words in result:
